...
describe('InventoryService', () => {
  ...
  describe('addItem', () => {
    it('should throw AppError if input is missing', async () => {
      ...
      await expect(service.addItem('Item', null as any, 10)).rejects.toThrow(AppError);
      await expect(service.addItem('Item', 1, null as any)).rejects.toThrow(AppError);
    });

    it('should call InventoryItem.create with correct parameters', async () => {
      const mockItem = { id: 1, name: 'Item', quantity: 1, price: 10 };
      const createSpy = jest.spyOn(inventoryModel.InventoryItem, 'create').mockResolvedValue(mockItem as any);
  ...
  describe('listItems', () => {
    it('should call InventoryItem.find and return results', async () => {
      ...
      const findSpy = jest.spyOn(inventoryModel.InventoryItem, 'find').mockResolvedValue(mockItems as any);
